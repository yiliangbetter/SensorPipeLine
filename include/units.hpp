...
using KilometersPerHour = Unit<Velocity, std::ratio<5, 18>>; // 1 km/h = 5/18 m/s
...
using Degrees = Unit<Angle, std::ratio<314159265, 18000000000>>; // π/180 approximation
...
constexpr Kilometers operator""_km(long double v) { return Kilometers(static_cast<double>(v)); }
constexpr Kilometers operator""_km(unsigned long long v) { return Kilometers(static_cast<double>(v)); }

constexpr Millimeters operator""_mm(long double v) { return Millimeters(static_cast<double>(v)); }
constexpr Millimeters operator""_mm(unsigned long long v) { return Millimeters(static_cast<double>(v)); }
...
constexpr Milliseconds operator""_ms(unsigned long long v) { return Milliseconds(static_cast<double>(v)); }

// Velocity
constexpr MetersPerSecond operator""_mps(long double v) { return MetersPerSecond(static_cast<double>(v)); }
constexpr MetersPerSecond operator""_mps(unsigned long long v) { return MetersPerSecond(static_cast<double>(v)); }

constexpr KilometersPerHour operator""_kph(long double v) { return KilometersPerHour(static_cast<double>(v)); }
constexpr KilometersPerHour operator""_kph(unsigned long long v) { return KilometersPerHour(static_cast<double>(v)); }
...
constexpr Radians operator""_rad(unsigned long long v) { return Radians(static_cast<double>(v)); }
...
std::ostream& operator<<(std::ostream& os, const Unit<Dimension, Ratio>& unit) {
    ...
    if constexpr (std::same_as<Dimension, Length>) {
        if constexpr (std::same_as<Ratio, std::ratio<1>>) os << " m";
        else if constexpr (std::same_as<Ratio, std::kilo>) os << " km";
        else if constexpr (std::same_as<Ratio, std::milli>) os << " mm";
        else os << " (length)";
    } else if constexpr (std::same_as<Dimension, Time>) {
        if constexpr (std::same_as<Ratio, std::ratio<1>>) os << " s";
        else if constexpr (std::same_as<Ratio, std::milli>) os << " ms";
        else os << " (time)";
        ...
        if constexpr (std::same_as<Ratio, std::ratio<1>>) os << " rad";
        else os << " deg";
    }
