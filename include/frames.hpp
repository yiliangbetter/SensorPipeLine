...
concept Frame = std::is_empty_v<T> &&
                (std::same_as<T, VehicleFrame> || std::same_as<T, LidarFrame> ||
                 std::same_as<T, CameraFrame> || std::same_as<T, RadarFrame> ||
                 std::same_as<T, MapFrame> || std::same_as<T, IMUFrame>);
...
struct Point3D {
  ...
  constexpr Point3D(Meters x_ = Meters(0), Meters y_ = Meters(0),
                    Meters z_ = Meters(0))
      : x(x_), y(y_), z(z_) {
  }
  ...
  constexpr Meters norm() const {
    return Meters(
        std::sqrt(x.value * x.value + y.value * y.value + z.value * z.value));
  }
};
...
struct Velocity3D {
  ...
  constexpr MetersPerSecond speed() const {
    return MetersPerSecond(std::sqrt(vx.value * vx.value + vy.value * vy.value +
                                     vz.value * vz.value));
  }
};
...
struct Transform {
  ...
  constexpr Transform(Point3D<To> trans,
  ...
  constexpr Point3D<To> operator()(const Point3D<From>& p) const {
    // p_to = R * p_from + t
    Meters x = Meters(rotation[0][0] * p.x.value + rotation[0][1] * p.y.value +
                      rotation[0][2] * p.z.value) +
               translation.x;

    Meters y = Meters(rotation[1][0] * p.x.value + rotation[1][1] * p.y.value +
                      rotation[1][2] * p.z.value) +
               translation.y;

    Meters z = Meters(rotation[2][0] * p.x.value + rotation[2][1] * p.y.value +
                      rotation[2][2] * p.z.value) +
               translation.z;
  ...
  constexpr Velocity3D<To> operator()(const Velocity3D<From>& v) const {
    MetersPerSecond vx = MetersPerSecond(rotation[0][0] * v.vx.value +
                                         rotation[0][1] * v.vy.value +
                                         rotation[0][2] * v.vz.value);

    MetersPerSecond vy = MetersPerSecond(rotation[1][0] * v.vx.value +
                                         rotation[1][1] * v.vy.value +
                                         rotation[1][2] * v.vz.value);

    MetersPerSecond vz = MetersPerSecond(rotation[2][0] * v.vx.value +
...
constexpr Transform<A, C> operator*(const Transform<A, B>& t_ab,
  ...
  Point3D<C> t_ac;
  t_ac.x = Meters(t_ab.rotation[0][0] * t_bc.translation.x.value +
                  t_ab.rotation[0][1] * t_bc.translation.y.value +
                  t_ab.rotation[0][2] * t_bc.translation.z.value) +
           t_ab.translation.x;

  t_ac.y = Meters(t_ab.rotation[1][0] * t_bc.translation.x.value +
                  t_ab.rotation[1][1] * t_bc.translation.y.value +
                  t_ab.rotation[1][2] * t_bc.translation.z.value) +
           t_ab.translation.y;

  t_ac.z = Meters(t_ab.rotation[2][0] * t_bc.translation.x.value +
...
template <Frame From, Frame To>
constexpr Transform<From, To> translation_transform(Meters x, Meters y,
                                                    Meters z) {
  return Transform<From, To>(Point3D<To>(x, y, z),
                             {{{1, 0, 0}, {0, 1, 0}, {0, 0, 1}}});
}
...
constexpr Transform<From, To> rotation_z_transform(Radians yaw) {
  ...
  return Transform<From, To>(Point3D<To>(),
                             {{{c, -s, 0}, {s, c, 0}, {0, 0, 1}}});
}
