...
int main() {
    ...
    Point3D<VehicleFrame> vehicle_point(15.0_m, 0.0_m, 1.5_m);
    ...
    Transform<LidarFrame, VehicleFrame> T_lidar_to_vehicle(
    ...
    Point3D<LidarFrame> obstacle_lidar(8.0_m, 2.0_m, 0.0_m);
    Point3D<VehicleFrame> obstacle_vehicle = T_lidar_to_vehicle(obstacle_lidar);
    
    std::cout << "Obstacle in Lidar frame: (" 
              << obstacle_lidar.x << ", " 
              << obstacle_lidar.y << ", " 
              << obstacle_lidar.z << ")\n";
    
    std::cout << "Obstacle in Vehicle frame: (" 
              << obstacle_vehicle.x << ", " 
              << obstacle_vehicle.y << ", " 
              << obstacle_vehicle.z << ")\n";
    ...
    auto T_radar_to_vehicle = rotation_z_transform<RadarFrame, VehicleFrame>(0.523_rad); // ~30 deg
    
    Point3D<RadarFrame> target_radar(10.0_m, 0.0_m, 0.0_m);
    Point3D<VehicleFrame> target_vehicle = T_radar_to_vehicle(target_radar);
    
    std::cout << "Target in Radar frame: (" 
              << target_radar.x << ", " 
              << target_radar.y << ", " 
              << target_radar.z << ")\n";
    
    std::cout << "Target in Vehicle frame: (" 
              << target_vehicle.x << ", " 
              << target_vehicle.y << ", " 
              << target_vehicle.z << ")\n";
    ...
    Transform<CameraFrame, LidarFrame> T_cam_to_lidar = 
        translation_transform<CameraFrame, LidarFrame>(0.5_m, 0.2_m, -0.1_m);
    
    Transform<CameraFrame, VehicleFrame> T_cam_to_vehicle = 
        T_cam_to_lidar * T_lidar_to_vehicle;
    
    Point3D<CameraFrame> feature_camera(5.0_m, 1.0_m, 0.0_m);
    Point3D<VehicleFrame> feature_vehicle = T_cam_to_vehicle(feature_camera);
    
    std::cout << "Feature in Camera frame: (" 
              << feature_camera.x << ", " 
              << feature_camera.y << ", " 
              << feature_camera.z << ")\n";
    
    std::cout << "Feature in Vehicle frame: (" 
              << feature_vehicle.x << ", " 
              << feature_vehicle.y << ", " 
              << feature_vehicle.z << ")\n";
    ...
    Velocity3D<VehicleFrame> ego_velocity(25.0_mps, 0.0_mps, 0.0_mps);
    Velocity3D<MapFrame> obstacle_velocity(15.0_mps, 5.0_mps, 0.0_mps);
    
    std::cout << "Ego velocity (Vehicle frame): " 
              << ego_velocity.vx << ", " 
              << ego_velocity.vy << "\n";
    
    std::cout << "Obstacle velocity (Map frame): " 
              << obstacle_velocity.vx << ", " 
    ...
    Point3D<LidarFrame> lidar_detection(12.0_m, -1.5_m, 0.5_m);
    Point3D<VehicleFrame> lidar_in_vehicle = T_lidar_to_vehicle(lidar_detection);
    
    // Radar detects same obstacle
    Point3D<RadarFrame> radar_detection(11.8_m, -1.6_m, 0.4_m);
    Point3D<VehicleFrame> radar_in_vehicle = T_radar_to_vehicle(radar_detection);
    
    std::cout << "Lidar detection (Vehicle): (" 
              << lidar_in_vehicle.x << ", " 
              << lidar_in_vehicle.y << ")\n";
    
    std::cout << "Radar detection (Vehicle): (" 
              << radar_in_vehicle.x << ", " 
              << radar_in_vehicle.y << ")\n";
    
    // Calculate distance between detections (fusion consistency check)
    auto dx = lidar_in_vehicle.x - radar_in_vehicle.x;
    auto dy = lidar_in_vehicle.y - radar_in_vehicle.y;
    auto fusion_error = Meters(std::sqrt(dx.value * dx.value + dy.value * dy.value));
